...
あなたは仮説生成エージェントです。データ分析結果と整合性チェック結果に基づいて、問題の仮説を生成してください。
...
以下のタイプの問題仮説を考慮してください：
1. 仕様不整合（specification_inconsistency）
2. パラメータ不適切（parameter_inappropriate）
3. 想定外動作（unexpected_behavior）
4. データ品質問題（data_quality_issue）
5. アルゴリズムバグ（algorithm_bug）
